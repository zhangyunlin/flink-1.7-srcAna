 ...
 * local模式运行flink应用，是在一个JVM进程中，将JobManager和TaskManager作为线程运行。
 * 运行的默认并行度为1，可以用过env.setParallelism(int)方式设置并行度
 *
...
public class LocalStreamEnvironment extends StreamExecutionEnvironment {
	 ...
	 * specified name.
	 *
	 * env调用execute()方法的时候，程序开始真正运行
	 *
	 * @param jobName
	 *            name of the job
	 * @return JobExecutionResult
	 * 			The result of the job execution, containing elapsed time and accumulators.
	 * 		JobExecutionResult继承了JobSubmissionResult
	 */
	@Override
	public JobExecutionResult execute(String jobName) throws Exception {

		// transform the streaming program into a JobGraph
		//将flink app转变为一个StreamGraph
		//StreamGraph是所有flink算子组成的DAG
		...
		jobGraph.setAllowQueuedScheduling(true);//默认支持Queue调度方式

		Configuration configuration = new Configuration();
		configuration.addAll(jobGraph.getJobConfiguration());
		configuration.setString(TaskManagerOptions.MANAGED_MEMORY_SIZE, "0");//localMode
		...
		//local模式运行，也会在本地启动一个miniCluster
		...
		try {
			...
			return miniCluster.executeJobBlocking(jobGraph);//任务执行
		}
		finally {//释放资源
